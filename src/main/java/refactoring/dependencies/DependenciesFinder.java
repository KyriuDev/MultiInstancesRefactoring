...
public class DependenciesFinder
{
	...
	private void optimizeCluster(final Cluster cluster)
	{
		...
		for (DependencyGraph dependencyGraph : originalDependencies.keySet())
		{
			...
			if (!new DependenciesAnalyzer().verifyDependencySet(dependencySet))
			{
				...
				System.out.println("Computed " + validGraphs.size() + " valid graphs.");
											...
											final ArrayList<DependencyGraph> alreadyComputedGraphs)
	{
		...
		//System.out.println("Number of already computed graphs: " + alreadyComputedGraphs.size());
		...
		for (Node node : originalNodes)
		{
			...
			{
				System.out.println("Nodes " + this.stringifyHashsets(nodesToLink) + " will be linked to node " + node.bpmnObject().id());
	...
	private DependencyGraph findOptimalGraph(final ArrayList<DependencyGraph> dependencyGraphs)
	{
		...
		for (; firstGraphIndex < dependencyGraphs.size(); firstGraphIndex++)
		{
			...
			//System.out.println(cluster.stringify(0));
		...
		//System.out.println("First execution time: " + executionTime);

		for (int i = firstGraphIndex + 1; i < dependencyGraphs.size(); i++)
		{
			final DependencyGraph currentDependencyGraph = dependencyGraphs.get(i);
			//System.out.println("Current dependency graph:\n\n" + currentDependencyGraph.stringify(0));
			...
			{
				System.out.println("Dependency (" + dependency.firstNode().bpmnObject().id() + "," + dependency.secondNode().bpmnObject().id() + ")");
			}*/
			...
			//System.out.println("Current clusters:\n\n" + currentDependenciesParser.mainCluster().stringify(0));
			...
			//System.out.println("Current execution time: " + currentExecutionTime);
		...
		System.out.println("Optimal dependency graph:\n\n" + optimalGraph.stringify(0));
		System.out.println("Optimal graph has an execution time of " + executionTime);
	...
	private Graph dependencyGraphToBpmnGraph(final Cluster cluster) throws BadDependencyException
	{
		...
		{
			System.out.println("Current abstract graph:\n\n" + abstractGraph.stringify(0));
		}
