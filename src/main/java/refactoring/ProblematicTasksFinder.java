...
public class ProblematicTasksFinder
{
	...
	public ArrayList<Node> findProblematicTasks()
	{
		...
		System.out.println("Found " + this.problematicTasks.size() + " problematic nodes:\n");
		for (Node node : this.problematicTasks)
		{
			System.out.println("- Node \"" + node.bpmnObject().id() + "\"\n");
		}
			...
			if (object instanceof Task)
			{
				...
				for (Resource problematicResource : this.problematicResources)
				{
					if (task.resourceUsage().resources().contains(problematicResource))
					{
						...
						System.out.println("Find restrictive nodes of "+  taskNode.bpmnObject().id());
						...
						list.add(triple);
						System.out.println("Triple: Task " + triple.first().bpmnObject().id() + " has " + finalRestrictiveNodes.size() + " restrictive nodes and a score of " + score);
						//break;
					}
				}
			}
	...
	private ArrayList<Node> finalizeRestrictiveNodes(final ArrayList<ArrayList<Node>> restrictiveNodes)
	{
		...
		for (ArrayList<Node> currentResNodes : restrictiveNodes)
		{
			System.out.println("Current res nodes:\n\n");

			for (Node node : currentResNodes)
			{
				System.out.println("- " + node.bpmnObject().id() + "\n");
			}
		}
	...
	{
		final ArrayList<Node> problematicTasks = new ArrayList<>();
		...
		while (!this.problematicResources.isEmpty())
		{
			...
			else
			{
				...
				System.out.println("Graph without parallelism:\n\n" + this.copiedGraph.toString());
									   ...
									   final HashSet<Node> unparallelizedRestrictiveNodes)
	{
		if (bestCandidate.second().isEmpty())
		{
			...
			System.out.println("Best candidate: " + bestCandidate.first().bpmnObject().id());
			final Node parallelGateway = this.findFurthestParallelSplit(bestCandidate.first(), null);
			System.out.println("Corresponding parallel gateway: " + parallelGateway.bpmnObject().id());

			//System.out.println("Graph before (end):\n" + this.copiedGraph.stringify());
			//System.out.println("Graph before (start):\n" + this.copiedGraph.toString());
			this.moveBlockOutOfParallelGateway(parallelGateway, bestCandidate.first(), bestCandidate.first());
			//System.out.println("Graph after (end):\n" + this.copiedGraph.stringify());
			//System.out.println("Graph after (start):\n" + this.copiedGraph.toString());
		}
		else
		{
			System.out.println("Best candidate: " + bestCandidate.first().bpmnObject().id());
			...
			for (int i = 0; i < restrictiveNodesToManage.size() - 1; i++)
			{
				...
				if (this.loopFinder.nodeIsInLoop(nodeToMove))
				{
					...
					{
						//Loop
						correspondingNode = this.copiedGraph.getNodeFromID(loop.exitPoint().bpmnObject().id());
					}
					else
					{
						//Choice
						correspondingNode = GraphUtils.findCorrespondingMergeGateway(nodeToMove.parentNodes().iterator().next());
					}
				}
				else
				{
				...
				this.moveBlockOutOfParallelGateway(currentParallelGateway, nodeToMove, correspondingNode);
			}
			...
			this.moveBlockOutOfParallelGateway(lastParallelGateway, nodeToMove, correspondingNode);
											   ...
											   final Node lastNode)
	{
		...
		if (parallelGateway.childNodes().size() > 2)
		{
			...
			System.out.println("ChildFlowChild has " + childFlowChild.parentNodes().size() + " parents.");
										   ...
										   final Node closestRestrictiveNode)
	{
		...
		else
		{
			if (this.loopFinder.nodeIsInLoop(closestRestrictiveNode))
			{
				...
				{
					//Choice
					parallelSplit = this.findFurthestEnclosingParallelGatewayV2(currentNode, new ArrayList<>(), new HashSet<>(), currentNode);
				}
			}
			else
			{
