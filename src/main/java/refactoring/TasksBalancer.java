...
public class TasksBalancer
{
						 ...
						 final ArrayList<Node> problematicNodes)
	{
		...
		System.out.println("Found " + this.problematicNodes.size() + " problematic nodes.");
	}
	...
	public Graph balanceTasks()
	{
		...
		for (AbstractGraph abstractGraph : this.mainCluster.abstractGraphs())
		{
			System.out.println("Abstract graph after problematization:\n\n" + abstractGraph.stringify(0));
		}
												...
												final HashMap<Cluster, Boolean> clusterProblematicity)
	{
		...
		//System.out.println("Possibly optimal graph:\n\n" + optimalGraph.initialNode().stringify(0, new ArrayList<>()));
		...
		//System.out.println("First graph exec time: " + optimalExecTime);
		...
		while (iterator.hasNext())
		{
			...
			//System.out.println("Current exec time: " + currentExecTime + " VS optimal exec time: " + optimalExecTime);
		...
		System.out.println("Final optimal graph:\n\n" + finalGraph.initialNode().stringify(0, new ArrayList<>()));
	}
	...
	private boolean generateAbstractGraphsCombinations(final Cluster cluster)
	{
		...
		System.out.println("Found " + abstractNodesToCorrect.size() + " abstract nodes to correct.");

		if (abstractNodesToCorrect.isEmpty())
		{
			if (cluster.abstractGraphs().size() == 1)
			{
				cluster.addPossiblyOptimalBalancedCluster(cluster.abstractGraphs().get(0));
			}
			else
			{
				...
				cluster.addPossiblyOptimalBalancedCluster(abstractGraph);
			}

			return false;
		}

		for (Pair<AbstractNode, AbstractNode> pair : abstractNodesToCorrect)
		{
			System.out.println("\n- " + pair.second().id());
		}
		...
		{
			//System.out.println("while infini");
		...
		System.out.println("Found " + finalCombinations.size() + " possibly optimal combinations.");

		for (AbstractGraph abstractGraph : finalCombinations)
		{
			System.out.println("Final dependencies combination:\n\n" + abstractGraph.stringify(0));
		}
		...
		System.out.println("Found " + nonDependentNonProblematicNodes.size() + " non dependent non problematic nodes.");
		System.out.println("Found " + nonDependentProblematicNodes.size() + " non dependent problematic nodes.");
		...
		cluster.addAllPossiblyOptimalBalancedClusters(fullyCombinedGraphs);
		return true;
	}
									  ...
									  final Cluster cluster)
	{
		for (Pair<AbstractNode, AbstractNode> incorrectNode : incorrectNodes)
		{
			...
			System.out.println("Number of reference graphs: " + generatedGraphs.size());
			...
			for (AbstractGraph referenceGraph : generatedGraphs)
			{
				...
				System.out.println("Found " + nonParallelizableFreeNodes.size() + " non parallelizable free nodes.");
				System.out.println("Found " + parallelizableFreeNodes.size() + " parallelizable free nodes.");
				...
				else
				{
					...
					{
						System.out.println("There is at least one subgraph containing non parallelizable tasks.");
						/*
						...
						//System.out.println("Copied pivot:\n\n" + copiedPivot.stringify(0));
						...
						for (Pair<AbstractNode, AbstractNode> combination : combinations)
						{
							...
							//System.out.println("Possible graph:\n\n" + newAbstractGraph.stringify(0));
						}

						//Merge all subgraphs together
						ArrayList<AbstractGraph> currentGraphsToMerge = possibleGraphs;

						System.out.println("Found " + currentGraphsToMerge.size() + " graphs to merge.");
				...
				for (final AbstractGraph possiblyOptimalVersionOfNode : possiblyOptimalVersionsOfCurrentNode)
				{
					...
					//System.out.println("First node: " + firstNode.stringify(0));
					//System.out.println("Last node: " + lastNode.stringify(0));
					...
					System.out.println("Current incorrect node: " + incorrectNode.second().id());
					...
					if (incorrectNode.first() != null)
					{
						...
						//System.out.println("current node has previous node(s)");
					...
					if (!incorrectNode.second().successors().isEmpty())
					{
						...
						//System.out.println("Current node has next node(s)");
						//System.out.println("Next node: " + incorrectNode.second().successors().iterator().next().id());
						//System.out.println("Copied graph: " + graphCopy.stringify(0));
			...
			break; //TODO ESSAYER DE COMPRENDRE POURQUOI CA NE MARCHE PAS SANS LE BREAK PLUTOT....
		}
	}
	...
	private ArrayList<AbstractGraph> mergeGraphWithGraph(final AbstractGraph graph1,
														 ...
														 final Cluster cluster)
	{
		System.out.println("First graph to merge:\n\n" + graph1.stringify(0));
		System.out.println("Second graph to merge:\n\n" + graph2.stringify(0));
		...
		else
		{
			...
			System.out.println("Found " + graph1Blocks.size() + " blocks for graph1");
			System.out.println("Found " + graph2Blocks.size() + " blocks for graph2");
			...
			if (graph1Blocks.size() > graph2Blocks.size())
			{
				for (int i = 0; i < graph2BlocksAndDuration.size(); i++)
				{
					...
					System.out.println("Block to link:\n\n" + blockToLink.first().stringify(0));
					System.out.println("Best graph:\n\n" + bestGraph.stringify(0));
		...
		for (AbstractGraph abstractGraph : mergedGraphs)
		{
													...
													final ArrayList<AbstractNode> graph2Nodes)
	{
		...
		{
			System.out.println("First nodes are not equal for graph 1: " + firstNode1.id() + " VS " + graph1Nodes.get(0).id());
		...
		{
			System.out.println("First nodes are not equal for graph 2: " + firstNode2.id() + " VS " + graph2Nodes.get(0).id());
			if (initialNode == null)
			{
		...
		{
			System.out.println("Only one combination");
		...
		{
			System.out.println("Several combinations");
		...
		{
			System.out.println("Last nodes are not equal for graph 1: " + lastCombinedNode1.id() + " VS " + graph1Nodes.get(graph1Nodes.size() - 1).id());
			int index = 0;
		...
		{
			System.out.println("Last nodes are not equal for graph 2: " + lastCombinedNode2.id() + " VS " + graph2Nodes.get(graph2Nodes.size() - 1).id());
			int index = 0;
	...
	private ArrayList<ArrayList<AbstractNode>> splitInParallelizableBlocks(final ArrayList<AbstractNode> nodes)
	{
		...
		{
			System.out.println("Abstract node is problematic: " + node.isProblematic());
																				  ...
																				  final Cluster cluster)
	{
		...
		if (HEURISTIC == Heuristic.COMBINATION)
		{
			...
			//System.out.println("Graph to merge with free nodes:\n\n" + graph.stringify(0));
			//System.out.println("Found " + parallelizableNodes.size() + " free nodes.");

			final Collection<Collection<Node>> parallelizableNodesCombinations = Utils.getCombinationsOf(parallelizableNodes);
			//System.out.println("Generated " + parallelizableNodesCombinations.size() + " combinations of parallelizable nodes.");

		/*for (Collection<Node> combination : parallelizableNodesCombinations)
		{
			System.out.print("\n\nCurrent combination: (");
			...
			//System.out.println("Found " + graphNodes.size() + " graph nodes.");
			...
			System.out.println("Found " + parallelizableBlocks.size() + " parallelizable blocks.");
			System.out.println("Found " + abstractNodesCombinations.size() + " combinations of abstract nodes.");
		...
		/*for (Pair<ArrayList<Pair<Collection<Node>, List<AbstractNode>>>, Collection<Collection<Node>>> combination : combinations)
		{
			System.out.println("\n\nCurrent combination has " + combination.first().size() + " nodes:");

			for (Pair<Collection<Node>, List<AbstractNode>> currentNode : combination.first())
			{
				System.out.print("\n- Current nodes: (");
				...
				System.out.print(") go in group of abstract nodes (");

				for (AbstractNode abstractNode : currentNode.second())
				{
					System.out.print(abstractNode.id() + ", ");
				}
		...
		System.out.println("\n\nFound " + combinations.size() + " possible combinations.");*/
		 ...
		 */
			...
			for (Pair<ArrayList<Pair<Collection<Node>, List<AbstractNode>>>, Collection<Collection<Node>>> combinationPair : combinations)
			{
				...
				//combinationNodes.remove(0);

				//System.out.println("Abstract graph in generation:\n\n" + abstractGraph.stringify(0));
				...
				//TODO MOST IMPORTANT TO DEBUG
				//System.out.println("Graph to merge with free nodes:\n\n" + graph.stringify(0));
				//System.out.println("Generated abstract graph in merge:\n\n" + abstractGraph.stringify(0));
			}
		}
		else
		{
												 ...
												 final ArrayList<AbstractNode> graphNodes)
	{
		...
		for (List<AbstractNode> abstractNodesCombination : abstractNodes)
		{
			for (Collection<Node> parallelizableNodesCombination : parallelizableNodes)
			{
				...
				//System.out.println("Abstract nodes list goes from size " + abstractNodes.size() + " to size " + copiedAbstractNodes.size());
				//System.out.println("Parallelizable nodes list goes from size " + parallelizableNodes.size() + " to size " + copiedParallelizableNodes.size());
	...
	private AbstractGraph findReferenceGraph(final AbstractNode abstractNode,
											 final Cluster cluster)
	{
		System.out.println("Abstract node to consider:\n\n" + abstractNode.stringify(0));
	...
	private AbstractGraph electPivot(final AbstractNode abstractNode)
	{
		...
		System.out.println("Pivot found: " + (pivot != null));
	...
	private boolean setAlreadyCorrectNodesPerGraph(final AbstractGraph abstractGraph)
	{
		...
		while (true)
		{
			...
			System.out.println("Current abstract node: " + currentNode.id());
			for (Node node : currentNode.listNodes())
			{
				System.out.println("Current node: " + node.bpmnObject().id());
				for (Node problematicNode : this.problematicNodes)
				{
					System.out.println("Current problematic node: " + problematicNode.bpmnObject().id());
					if (node.equals(problematicNode))
					{
			...
			if (abstractNodeContainsProblematicNodes)
			{
				...
				System.out.println("Found problematic abstract node");
				currentNode.setProblematic();
			}
			else
			{
				System.out.println("Found non problematic abstract node");
				currentNode.setCorrected();
			}
	...
	private void computeSubgraphsTasks(final Cluster cluster)
	{
		for (AbstractGraph graph : cluster.abstractGraphs())
		{
			...
			System.out.println("Graph has " + graph.allNodes().size() + " nodes.");
		}
