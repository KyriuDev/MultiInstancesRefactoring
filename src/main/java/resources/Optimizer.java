...
public class Optimizer
{
	...
	public Optimizer(final ArrayList<BpmnProcessObject> objects,
					 ...
					 final int iat)
	{
		this.interval = 1; //this.computeInterval(objects);
		//System.out.println("Computed interval: " + this.interval);
		this.graph = graph;
	...
	public int computeProcessExecutionTime()
	{
		...
		//this.graph.clearParallelGatewaysTime();
	...
	public ResourcePool computeMaximalPoolForNProcesses()
	{
		...
		//System.out.println("\nTasks executing at any instant of time for any number n of processes:");
		//this.printTasksPerInstantOfTime(multiProcTasksPerInstantOfTime);
	...
	{
		final HashMap<Integer, ArrayList<Pair<Task, Double>>> tasksPerInstantOfTime = this.computeTasksPerInstantOfTime();
		this.printTasksPerInstantOfTime(tasksPerInstantOfTime);
	...
	public HashMap<Resource, Integer> computeProcessAbsorbance(final ResourcePool realPool)
	{
		...
		for (int time : this.resourcesPerInstantOfTime.keySet())
		{
			...
			for (Resource resource : globalResourceSet.resourcesSet())
			{
				...
				//if (resource.name().equals("assistant")) System.out.println("Current usage : " + currentUsage + " / Max usage : " + maxUsage);
		...
		for (Resource resource : exceedingUsage.keySet())
		{
			...
			final int absorbance = (int) (((double) totalOveruse / ((double) totalUnderuse * (double) 10)) * 100); //TODO REVOIR
			absorbanceMap.put(resource, absorbance);
			System.out.println("Resource " + resource.name() + " has " + absorbance + "% of absorbance.");
		}
	...
	private HashMap<Integer, ArrayList<Pair<Task, Double>>> computeMultiProcTasksPerInstantOfTime(final HashMap<Integer, ArrayList<Pair<Task, Double>>> tasksPerInstantOfTime)
	{
		...
		System.out.println("Tasks per instant of time: " + tasksPerInstantOfTime.size());
	...
	private HashMap<Integer, ArrayList<Pair<Task, Double>>> computeTasksPerInstantOfTime()
	{
		...
		//System.out.println("Graph is null: " + (this.graph == null));
		...
		//System.out.println("\nRaw tasks per instant of time:");
		//this.printTasksPerInstantOfTime(tasksPerInstantOfTime);

		this.correctTasks(tasksPerInstantOfTime, false);

		//System.out.println("\nCorrected tasks per instant of time:");
		//this.printTasksPerInstantOfTime(tasksPerInstantOfTime);
																				...
																				final Node correspondingMergeGateway)
	{
		...
		if (executionTimes.isEmpty())
		{
			...
			for (Node splitChild : parallelSplitGateway.childNodes())
			{
				...
				//System.out.println("Before computing path execution time");
				this.computePathExecutionTimes(splitChild, correspondingMergeGateway, freshPair, childPathsList);
				//this.printChildPathsPairs(childPathsList, splitChild.childNodes().iterator().next());
				//System.out.println("After computing path execution time");
				...
				//this.printParallelGatewayExecutionTimes(temporaryMap, splitChild.childNodes().iterator().next());
			}

			executionTimes.putAll(this.mergeHashMapsV2(temporaryMaps));
		}

		//this.printParallelGatewayExecutionTimes(executionTimes, parallelSplitGateway);
										   ...
										   final ArrayList<Pair<Integer, Double>> pairList)
	{
		...
		//System.out.println(currentNode.bpmnObject().toString());
		...
		if (currentNode.bpmnObject() instanceof Task)
		{
			...
			//System.out.println("Found task |" + currentNode.bpmnObject().id() + "|. New execution time is |" + newExecutionTime + "|.");
		}
		...
		else if (currentNode.bpmnObject() instanceof SequenceFlow)
		{
			...
			//System.out.println("Found sequence flow. Flow probability is |" + currentNode.bpmnObject().probability() + "|. New probability is |" + newProbability + "|.");
		}
		else
		{
	...
	private HashMap<Integer, Double> mergeHashMapsV2(final ArrayList<HashMap<Integer, Double>> hashmaps)
	{
		...
		while (candidate != -1)
		{
			...
			for (int i = 0; i < hashmaps.size(); i++)
			{
				...
				if (mainHashmap.containsKey(candidate))
				{
					...
					for (int j = 0; j < hashmaps.size(); j++)
					{
						...
						//System.out.println("Local proba: " + localProba);
			...
			//System.out.println("Adding couple (" + candidate + "/" + globalProba + ") to map.");
		...
		for (Integer executionTime : tasksCopy.keySet())
		{
			...
			for (Pair<Task, Double> currentTask : tasksAtCurrentTime)
			{
				...
				else
				{
					...
					//Put 0.01 of margin for double computations rounding errors
					/*if (!multiInstances
						&& newProbability > 1.01d)
					{
						throw new IllegalStateException("Probability can not exceed 1. Got |" + newProbability + "| for task |" + duplicate.first().id() + "| at time |" + executionTime + "|.");
					}*/
	...
	/**
	 * Debugging function used to print properly all the tasks executing at any given time
	 * t of the execution of the process.
	 *
	 * @param tasks a map containing as key an execution time, and as value a list of pair task/probability of execution
	 */
	private void printTasksPerInstantOfTime(final HashMap<Integer, ArrayList<Pair<Task, Double>>> tasks)
	{
		final StringBuilder builder = new StringBuilder("Execution flow of the current BPMN process:");

		int currentTime = 0;

		//Weird iteration over the map, but needed to order the timestamps
		while (true)
		{
			//Should never be null
			final ArrayList<Pair<Task, Double>> taskAtCurrentTime = tasks.get(currentTime);

			if (taskAtCurrentTime == null)
			{
				break;
			}
			...
			for (Pair<Task, Double> taskWithProbability : taskAtCurrentTime)
			{
				builder
						.append("\n		- ")
						.append(taskWithProbability.first().id())
						.append(" (")
						.append(taskWithProbability.second())
						.append(")");
			}
	...
	/**
	 * Debugging function used to print properly the list of possible execution times
	 * of a path starting from a child node of a parallel split gateway.
	 *
	 * @param pairs the list of pairs execution time/probability for the current path
	 * @param child the starting node of the current path
	 */
	private void printChildPathsPairs(final ArrayList<Pair<Integer, Double>> pairs,
									  final Node child)
	{
		final StringBuilder builder = new StringBuilder();
		builder
				.append("\nPath starting with child |")
				.append(child.bpmnObject().id())
				.append("| can execute in:");
	...
	/**
	 * Debugging function used to print properly the list of resources needed by the process
	 * to execute without waiting time at any given time t of the execution.
	 *
	 * @param resourcePools a map containing as key an execution time and as value the resource pool needed at this time
	 */
	private void printResourcePoolsPerInstantOfTime(final HashMap<Integer, ResourcePool> resourcePools)
	{
		final StringBuilder builder = new StringBuilder("\nResource pools per instant of time:");
		...
		{
			//Should never be null
			final ResourcePool resourcePool = resourcePools.get(currentTime);
			...
			for (final Resource resource : this.globalResourceSet.resourcesSet())
			{
				builder
						.append("\n		- ")
						.append(resourcePool.getUsageOf(resource))
						.append(" ")
